...
init = function() {
  ...
  return jsx.LayoutPreviewGenerator = (function() {
    ...
    _generateNode = function(node, parentName, indentLevel, doc, layers, $root, $element, result, offsetX, offsetY, component, isRoot) {
      var $$, $div, childContainer, config, ref, tag;
      ...
      return _appendComponentElement(config.data, _cheerio, _components, $element, $div);
    };

    _appendNodeOption = function(option, childContainer, node, classPath, indent, doc, layers, $root, result, component) {
      if (option.embed) {
        ...
        return _generateNodeList(node.children, classPath, indent + 1, doc, layers, $root, childContainer, result, 0, 0, component, false);
      }
    };

    _appendComponentElement = function(data, _cheerio, _components, $element, $div) {
      if (_componentExportable && data.option.component) {
        return _appendComponent(_cheerio, _components, data.option.component);
      } else {
        return $element.append($div);
      }
    };

    _createResultCSS = function(config, isComponentRoot, result, indent, node, layers, offsetX, offsetY) {
      var component, css;
    ...
    _appendComponent = function(cheerio, componentList, componentName) {
      var $component;
      componentList.push({
        id: config.id,
        name: componentName,
