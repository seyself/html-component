...
var RESTClient, init, jsx, nib, path, stylus;
...
init = function() {
  ...
  return jsx.LayoutPreviewGenerator = (function() {
    var $, _appendCSSElementIndent, _appendComponent, _appendComponentElement, _appendNodeOption, _appendReferenceNode, _assets, _compareNodeHeight, _componentExportable, _components, _copyComponentAssets, _copyHTMLAssets, _createComponentCSS, _createComponentFilePath, _createComponentFiles, _createComponents, _createCopyList, _createDestDir, _createElementCSS, _createElementTag, _createFilePath, _createHTML, _createHTMLCode, _createPackageJson, _createResultCSS, _createSrcFiles, _createStyleSheet, _createTextElementCSS, _data, _deleteQuartFromURLText, _exportCssFile, _exportHTML, _exportJadeFile, _generateCSS, _generateInit, _generateNode, _generateNodeList, _generateRelativeLayout, _getAssetFilePathList, _getBasicStyle, _getIndent, _getMetaData, _getStylePSE, _getText, _isComponent, _isComponentDebugJS, _isFullURL, _isIgnoreAssetFilePath, _isPositionRelative, _layerNameIsLink, _makeArgumentList, _makeAssetPathList, _packageJsonTemplate, _params, _ref_elements, _replaceAssetPath, _replaceJadeFormat, _setAssetPathList, _setClassPath, _setComponentFiles, _setConfig, _setDataBackground, _setDataPosition, _writePackageJsonTemplate;
    ...
    LayoutPreviewGenerator.prototype.generate = function(params) {
      ...
      return _exportHTML(params, filePath, exportData);
    };

    _generateInit = function(params) {
      $ = _cheerio.load('<div><div id="main" class="_' + _data.document.filename + '"></div></div>', {
        decodeEntities: false
      });
      params.dest = '../app/build';
      _components = [];
      return _ref_elements = {};
    };

    _createStyleSheet = function(jsonData) {
      var $main, params, styleCode;
      $main = $('div#main');
      params = {};
      styleCode = _getBasicStyle(jsonData.document);
      _generateRelativeLayout(jsonData, $main, params);
      return styleCode += params.css;
    };

    _createHTMLCode = function(document, filePath) {
      var $body, body, htmlCode;
      $body = $('div');
      body = $body.html();
    ...
    _exportHTML = function(params, filePath, data) {
      return _createDestDir(params.dest, function() {
        fs.writeFile(filePath.htmlFile, data.html, {
          encoding: 'utf8'
        }, null);
        return _copyHTMLAssets(params, data.html, function() {
          return _exportJadeFile(params, data.html, filePath.jadeFile, filePath.stylusFile, data.style, filePath.cssFile);
        });
      });
    };

    _createFilePath = function(params) {
      var cssFile, cssPath, htmlFile, jadeFile, jsPath, stylusFile;
    ...
    _exportJadeFile = function(params, html, jadeFile, stylusFile, style, cssFile) {
      if (params.export_jade) {
        return exec('mkdir -p ' + './src/pages', function() {
          return html2jade.convertHtml(html, {
            ...
            return _exportCssFile(params, stylusFile, style, cssFile);
          });
        });
      } else {
        if (_componentExportable) {
          return _createComponents(params);
        }
      }
    };

    _exportCssFile = function(params, stylusFile, style, cssFile) {
    ...
    _copyHTMLAssets = function(params, html, callback) {
      var copyList, matches, pathes;
      pathes = [];
    ...
    _createCopyList = function(params, pathes) {
      var copyList, dstDir, j, len1, src, srcDir, src_diff_1, src_diff_2;
      copyList = [];
    ...
    _createComponents = function(params) {
      ...
      if (data) {
        ...
        return _setComponentFiles(params, data);
      }
    };
    ...
    _setComponentFiles = function(params, data) {
      var cssFile, html;
      html = data.node.html();
    ...
    _createPackageJson = function(params, data) {
      var filePath, json;
    ...
    _replaceAssetPath = function(params, data, html) {
      ...
      if (matches != null) {
        matches.forEach(function(code) {
          return pathes = _makeAssetPathList(code, pathes);
        });
      }
      for (j = 0, len1 = pathes.length; j < len1; j++) {
        src = pathes[j];
    ...
    _setAssetPathList = function(dstBase, params, src, result) {
    ...
    _createComponentFiles = function(data, params, callback) {
      ...
      return exec('mkdir -p ' + filePath.dstDir, function() {
        fs.writeFileSync(filePath.htmlFile, params.html, {
          encoding: 'utf8'
        });
        _generateCSS(filePath.cssFile, filePath.style);
        if (params.export_jade) {
          return _createSrcFiles(filePath, params, callback);
        } else {
          return callback();
        }
      });
    ...
    _createSrcFiles = function(filePath, params, callback) {
      return exec('mkdir -p ' + filePath.srcDir, function() {
        return html2jade.convertHtml(params.html, {
          donotencode: true
        }, function(err, jade) {
          jade = _replaceJadeFormat(jade);
          fs.writeFileSync(filePath.jadeFile, jade, {
            encoding: 'utf8'
          });
          fs.writeFileSync(filePath.stylFile, filePath.style, {
            encoding: 'utf8'
          });
          return callback();
        });
    ...
    _createComponentFilePath = function(data) {
    ...
    _generateNode = function(node, parentName, indentLevel, doc, layers, $root, $element, result, offsetX, offsetY, component, isRoot) {
      var $$, $div, argumentList, childContainer, config, ref, tag;
      ...
      $div = $$('div');
      childContainer = $div;
      if ((ref = config.option) != null ? ref.link_url : void 0) {
        childContainer = $$('a');
      }
      ...
      return _appendComponentElement(argumentList, _cheerio, _components, $element, $div);
    };

    _makeArgumentList = function(config, result, indentLevel, node, layers) {
      return {
        config: config,
        result: result,
    ...
    _appendNodeOption = function(list, childContainer, doc, $root, component) {
      var option;
      option = list.config.option;
      if (option.embed) {
        return childContainer.append(unescape(option.embed));
      } else if (_layerNameIsLink(option.layer_name)) {
        return _appendReferenceNode(childContainer, _ref_elements, option.layer_name);
      } else {
        return _generateNodeList(list.node.children, list.config.classPath, list.indentLevel + 1, doc, list.layers, $root, childContainer, list.result, 0, 0, component, false);
      }
    };

    _appendComponentElement = function(list, _cheerio, _components, $element, $div) {
      if (_componentExportable && list.config.data.option.component) {
        return _appendComponent(list.config.id, _cheerio, _components, list.config.data.option.component, $div, list.result, $element);
      } else {
        return $element.append($div);
      }
    };

    _createResultCSS = function(list, isComponentRoot, offsetX, offsetY) {
      var component, css;
      if (_componentExportable && list.config.data.option.component) {
    ...
    _appendComponent = function(id, cheerio, componentList, componentName, $div, result, $element) {
      var $component;
      componentList.push({
        id: id,
        name: componentName,
      ...
      $component = cheerio.load('<' + componentName + '></' + componentName + '>', {
        decodeEntities: false
      })(componentName);
      return $element.append($component);
    };

    _layerNameIsLink = function(layerName) {
      return layerName.match(/^@\d+/) !== null;
    };

    _appendReferenceNode = function(container, elements, layerName) {
      var ref_id, ref_node;
      ref_id = layerName.match(/^@(\d+)/)[1];
      ref_node = elements[ref_id];
      if (ref_node) {
        return container.append(ref_node.html());
      }
    };

    _setConfig = function(node, layers, parentName) {
      var className, classPath, data, id, option;
      id = node.id;
      data = layers[id];
      option = data.option;
    ...
    _setDataPosition = function(data, meta, offsetX, offsetY) {
      data.top = meta.position.relative.y + offsetY;
      return data.left = meta.position.relative.x + offsetX;
    };

    _setDataBackground = function(data, css, _indent) {
    ...
    _appendCSSElementIndent = function(_indent, css, classPath) {
      if (_indent) {
        css += _indent + '& > .' + classPath + '\n';
      } else {
        css += _indent + '.' + classPath + '\n';
    ...
    _createTextElementCSS = function(meta, indent, css) {
      if (meta.text) {
