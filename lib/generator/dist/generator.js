...
init = function() {
  ...
  return jsx.LayoutPreviewGenerator = (function() {
    var $, _assets, _compareNodeHeight, _componentExportable, _components, _copyComponentAssets, _copyHTMLAssets, _createComponentCSS, _createComponentFilePath, _createComponentFiles, _createComponents, _createDestDir, _createElementCSS, _createElementTag, _createFilePath, _createHTML, _createHTMLCode, _createPackageJson, _createStyleSheet, _createTextElementCSS, _data, _deleteQuartFromURLText, _exportCssFile, _exportHTML, _exportJadeFile, _generateCSS, _generateInit, _generateNode, _generateNodeList, _generateRelativeLayout, _getAssetFilePathList, _getBasicStyle, _getIndent, _getMetaData, _getStylePSE, _getText, _isComponent, _isComponentDebugJS, _isFullURL, _isIgnoreAssetFilePath, _isPositionRelative, _makeAssetPathList, _makeComponent, _packageJsonTemplate, _params, _ref_elements, _replaceAssetPath, _replaceJadeFormat, _setAssetPathList, _setClassPath, _setComponentFiles, _setEmbed, _writePackageJsonTemplate;
    ...
    _generateNode = function(node, parentName, indentLevel, doc, layers, $root, $element, result, offsetX, offsetY, component, isRoot) {
      var $$, $copm, $div, childContainer, className, classPath, cname, css, data, id, isComponentRoot, meta, option, tag;
      id = node.id;
      data = layers[id];
      option = data.option;
      meta = data.meta;
    ...
    _setEmbed = function(option, childContainer, _ref_elements, node, classPath, indentLevel, doc, layers, $root, result, component) {
      var ref_id, ref_node;
      if (option.embed) {
        ...
        if (ref_node) {
          return childContainer.append(ref_node.html());
        }
      } else {
        return _generateNodeList(node.children, classPath, indentLevel + 1, doc, layers, $root, childContainer, result, 0, 0, component, false);
      }
    };
