...
var INDENT_STR, RESTClient, RE_ASSET_FILE, _cheerio, _moduleDir, beautify, exec, fs, html2jade, htmlTemplate, jsx, nib, path, stylus, stylusTemplate;
...
jsx.LayoutPreviewGenerator = (function() {
  var $, _assets, _componentExportable, _components, _copyComponentAssets, _copyHTMLAssets, _createComponentCSS, _createComponentFiles, _createComponents, _createDestDir, _createElementCSS, _createElementTag, _createHTML, _createPackageJson, _createTextElementCSS, _data, _generateCSS, _generateNode, _generateNodeList, _generateRelativeLayout, _getBasicStyle, _getIndent, _getMetaData, _getStylePSE, _getText, _isPositionRelative, _packageJsonTemplate, _params, _ref_elements, _replaceAssetPath, _replaceJadeFormat;
  ...
  _createDestDir = function(dest, callback) {
    return exec('mkdir -p ' + path.join(dest, 'js'), function() {
      return exec('mkdir -p ' + path.join(dest, 'css'), function() {
        return exec('mkdir -p ' + 'src/pages/js', function() {
          return exec('mkdir -p ' + 'src/pages/css', function() {
            return exec('mkdir -p ' + 'src/assets', function() {
              return exec('mkdir -p ' + 'src/libs', function() {
                if (callback) {
                  return callback();
                }
              });
  ...
  _createComponents = function() {
    var cssFile, data, html, isWritable, packageJson, packageJsonPath, tmplPath;
