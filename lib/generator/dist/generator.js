...
jsx.LayoutPreviewGenerator = (function() {
  ...
  _generateRelativeLayout = function(params, $main, result) {
    ...
    offsetY = doc.offsetY;
    result.css = '';
    return _generateNodeList(indexes.children, doc, layers, $main, $main, result, doc.offsetX, doc.offsetY, '_' + doc.filename);
  };

  _generateNodeList = function(list, doc, layers, $root, $element, result, offsetX, offsetY, component) {
    var child, j, len1, results;
    results = [];
    for (j = 0, len1 = list.length; j < len1; j++) {
      child = list[j];
      if (child.enabled) {
        results.push(_generateNode(child, doc, layers, $root, $element, result, offsetX, offsetY, component));
      } else {
        results.push(void 0);
      }
    }
  ...
  _generateNode = function(node, doc, layers, $root, $element, result, offsetX, offsetY, component) {
    var $$, $copm, $div, childContainer, cname, css, data, id, isComponentRoot, meta, option, ref_id, ref_node, tag;
    id = node.id;
    data = layers[id];
    option = data.option;
    meta = data.meta;
  ...
  _createElementCSS = function(id, node, layers, offsetX, offsetY, component, isComponentRoot) {
    var bg_url, bg_x, bg_y, css, data, isRelative, meta, parent, translateX, translateY;
    data = layers[id];
    meta = data.meta;
    css = '';
    if (component) {
