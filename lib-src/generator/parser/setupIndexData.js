...
export function setupIndexData(layouts, options)
{
...
{
  let len = childIndexes.length;
  var bottomBorder = 0;
  for(var i=0; i<len; i++){
    let childIndex = childIndexes[i];
...
{
  let len = childIndexes.length;
  let isPositionRelative = _isPositionRelative(childIndexes);
  var prev_id = null;
  var next_id = null;
  for(var i=0; i<len; i++){
    let childIndex = childIndexes[i];
